...
namespace ipclib {
    ...
    template<class T> Result SharedMem<T>::create(const std::string& theName, const bool toCreate, const Protection theProtection) {
        ...
        int fd;
        if( (fd = shm_open(theName.c_str(), oflag, DEFAULT_PERMISSION)) == -1 ) return Result(errno, strerror(errno));
        ...
        if( (obj_addr = (T*)(mmap(NULL, sizeof(T), prot_oflag, MAP_SHARED, fd, 0))) == MAP_FAILED ) return Result(errno, strerror(errno));
